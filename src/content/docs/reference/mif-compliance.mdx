...
The capability ships as a self-contained plugin under `.github/mif/` with vendored,
checksum-pinned MIF schemas.

## Select the level

Edit the one config file, `.github/mif/config.yml`:
...
```bash
npm run validate:mif                 # uses .github/mif/config.yml
...
`.github/mif/schema/` mirrors MIF `develop/v1.0.0` verbatim; `.github/mif/VENDOR.lock`
