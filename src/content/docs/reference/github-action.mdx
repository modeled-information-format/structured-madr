...
Set `mode: mif` to validate ADRs as MIF (Modeled Information Format) instead of (or
in addition to) the MADR frontmatter check. The level is read from the consumer
repo's `.github/mif/config.yml` unless overridden.
...
Outputs in this mode: `mif-valid`, `mif-total`, `mif-passed`, `mif-failed`. See the
[MIF Compliance reference](/reference/mif-compliance/) for the level model and field mapping.
